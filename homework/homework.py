...
def clean_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Eliminar registros con información no disponible (0 significa N/A)
    new_df = new_df.loc[new_df["MARRIAGE"] != 0]
    new_df = new_df.loc[new_df["EDUCATION"] != 0]
    
    # Agrupar valores > 4 en la categoría 4 (others)
...
def create_pipeline(x: pd.DataFrame) -> Pipeline:
    ...
    # El paso 3 solo pide transformar categóricas con OHE y luego Random Forest
...
def create_estimator(pipeline: Pipeline, x: pd.DataFrame) -> GridSearchCV:
    # Expandimos los hiperparámetros para permitir que Random Forest memorice 
    # y optimice mejor el patrón del conjunto de entrenamiento.
    param_grid = {
...
def main():
    ...
    # Asegurar que la carpeta de salida exista antes de escribir el archivo JSON
    os.makedirs(path3, exist_ok=True)
    with open(os.path.join(path3, 'metrics.json'), 'w') as file:
